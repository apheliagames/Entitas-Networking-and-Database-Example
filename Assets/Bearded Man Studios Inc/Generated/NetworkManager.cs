...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] DatabaseNetworkObject = null;
		public GameObject[] EntitasNetworkObject = null;
		public GameObject[] ExampleProximityPlayerNetworkObject = null;
		public GameObject[] MasterNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is DatabaseNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (DatabaseNetworkObject.Length > 0 && DatabaseNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(DatabaseNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
			...
			else if (obj is EntitasNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (EntitasNetworkObject.Length > 0 && EntitasNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(EntitasNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
			...
			else if (obj is MasterNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (MasterNetworkObject.Length > 0 && MasterNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(MasterNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateDatabase instead, its shorter and easier to type out ;)")]
		public DatabaseBehavior InstantiateDatabaseNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DatabaseNetworkObject[index]);
			var netBehavior = go.GetComponent<DatabaseBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<DatabaseBehavior>().networkObject = (DatabaseNetworkObject)obj;

			FinalizeInitialization(go, netBehavior, obj, position, rotation, sendTransform);
			
			return netBehavior;
		}
		[Obsolete("Use InstantiateEntitas instead, its shorter and easier to type out ;)")]
		public EntitasBehavior InstantiateEntitasNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(EntitasNetworkObject[index]);
			var netBehavior = go.GetComponent<EntitasBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<EntitasBehavior>().networkObject = (EntitasNetworkObject)obj;
		...
		[Obsolete("Use InstantiateMaster instead, its shorter and easier to type out ;)")]
		public MasterBehavior InstantiateMasterNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(MasterNetworkObject[index]);
			var netBehavior = go.GetComponent<MasterBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<MasterBehavior>().networkObject = (MasterNetworkObject)obj;
		...
		public DatabaseBehavior InstantiateDatabase(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DatabaseNetworkObject[index]);
			var netBehavior = go.GetComponent<DatabaseBehavior>();
		...
		public EntitasBehavior InstantiateEntitas(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(EntitasNetworkObject[index]);
			var netBehavior = go.GetComponent<EntitasBehavior>();
		...
		public MasterBehavior InstantiateMaster(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(MasterNetworkObject[index]);
			var netBehavior = go.GetComponent<MasterBehavior>();
